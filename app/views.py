...
def editar_carro(request, pk, templete_name='carro_form.html'):
...
def remover_carro(request, pk, templete_name='carro_delete.html'):
    carro = Carro.objects.get(pk = pk)
    if request.method == "POST":
        carro.delete()
        return redirect('carro_list')
    return render(request, templete_name, {'carro':carro})
